...
if status < 0 
	error ( 'SNCTOOLS:nc_get_attribute_struct:inqAttNameFailure', ...
        'mexnc:inq_attname failed on varid %d.',varid);
end
...
if status < 0 
	error ( 'SNCTOOLS:nc_get_attribute_struct:inqAttTypeFailure', ...
        'mexnc:inq_att failed on varid %d, attribute %s.',varid,attname);
end
...
if status < 0 
	error ( 'SNCTOOLS:nc_get_attribute_struct:getAttFailure', ...
        'mexnc:attget failed on varid %d, attribute %s.',varid,attname);
end
