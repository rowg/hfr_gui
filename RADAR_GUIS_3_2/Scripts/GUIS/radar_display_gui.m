function radar_display_gui(time, setupfile)
...
tpre = 'TOTm';                         % standard prefix for total file
srpre = '';                            % user defined prefix for radial file
stpre = '';                            % user defined prefix for total file
...
maxcolor = 120;                        % maximum velocity on color scale for m_vec (cm/s)
...
[cxmin, cxmax, cymin, cymax] = deal([],[],[],[]);
 ...
 % Create and hide the GUI figure as it is being constructed.
   ...
   %eph(5) = uimenu(pmh,'Label','Next Time','Callback',{@fwdbutton_Callback});
   %eph(6) = uimenu(pmh,'Label','Previous Time','Callback',{@bckbutton_Callback});
   %eph(7) = uimenu(pmh,'Label','Scale Up','Callback',{@scaleupbutton_Callback});
   %eph(8) = uimenu(pmh,'Label','Scale Down','Callback',{@scaledownbutton_Callback});
   ...
   exh(3) = uimenu(xmh,'Label','Range Ring','Callback',{@rangeringbutton_Callback});
   exh(4) = uimenu(xmh,'Label','Save Scale','Callback',{@savescalebutton_Callback});
   exh(5) = uimenu(xmh,'Label','Load New Setup File','Callback',{@loadnewsetup_Callback});
...
function setpath_Callback(novar,novar1) 
...
function pc_Callback(novar,novar1) 
...
function date_Callback(novar,novar1) 
...
function station_Callback(novar,novar1)
...
function radtype_Callback(novar,novar1)     
   if strcmp(get(tottool,'State'),'on'); 
    stpre = inputdlg('Enter Prefix For TOTAL Vector File','Prefix Selection',1,{'TOTe'});
    stpre = char(stpre);
   else        
...
function radtypestd_Callback(novar,novar1)  
   srpre = ''; stpre = '';
   display_settings;  
end
...
function plotbutton_Callback(novar,novar1)
...
function plot_Callback(novar,novar1) 
% Plots radial or total vectors.  
 
set(0,'CurrentFigure',f);
if strcmp(get(threddstool,'State'),'on'); % PLOT RUTGER'S OI TOTALS
 ...
 % --------------------------- BEGIN LOAD DATA SECTION ------------------------- %
  ...
  if isfield(DATA,'ErrorEstimates')
      try
       ...
       DATA = subsrefTUV( DATA, ikeep, ':', 0, 0 ); 
          ...
          disp('Did not find error estimates.  No GDOP mask applied.')
      end
  end
  ...
  if strcmp(get(qtool,'State'),'on')
         plothandles(pcount) = m_quiver(DATA.LonLat(:,1), DATA.LonLat(:,2), DATA.U(:).*scq,DATA.V(:).*scq,0); 
         ...
         if strcmp(get(tottool,'State'),'off') && strcmp(get(rbtool,'State'),'on'); % for radials with red/blue settings
           ...
           pcount = pcount + 1;
           plothandles(pcount) = m_quiver(DATA.LonLat(to,1), DATA.LonLat(to,2), DATA.U(to).*scq,DATA.V(to).*scq,0);
...
function clearbutton_Callback(novar,novar1)
   % Clears the plot. 
...
function scalevaluebutton_Callback(source,eventdata)
% Allows user to set scale for arrows.
  if strcmp(get(qtool,'State'),'on')
    temp = str2num(char(inputdlg(sprintf('Enter new scaling factor.  Current value is %7.5f',scq),'Name')));
    if ~isempty(temp)
        scq = temp;
    end
  else
    temp = str2num(char(inputdlg(sprintf('Enter new scaling factor.  Current value is %7.5f',sc),'Name')));
    if ~isempty(temp)
        sc = temp;
    end
  end
...
function colorbutton_Callback(novar,novar1) 
   myhandle = input(['Enter plot number. 1-',num2str(pcount-1),' ']);
   setcolor = uisetcolor();
   set(plothandles(myhandle),'Color',setcolor);
end

function maxcolorbutton_Callback(novar,novar1)
   maxcolor = inputdlg('Enter Colorbar Maximum Velocity (cm/s) ','Set Colorbar');
   maxcolor = str2num(char(maxcolor));
end

% ACTION CALLBACKS

function editbutton_Callback(novar,novar1) 
% Hand edit the radial map.
     ...
     ee = 1;
     ei = [];
     [novar] = input('Adjust zoom and press enter when ready.');
...
function autoeditbutton_Callback(novar,novar1) 
   % calls gui to compute total
   autoedit_gui(dnum);
end

function totalbutton_Callback(novar,novar1) 
   % calls gui to compute total
   maketotal_gui(dnum);
end
...
% Assumes default processing settings for totals.
     ...
     [novar] = input('Zoom in and press enter to select total.');
     [gx,gy] = ginput(1);
     ...
     makeTotals_checktotal(newTUV.RADS,'Grid',grid,'TimeStamp',newTUV.RADS(1).TimeStamp, ...
                   ...
                   'CreationInfo','Teresa Garner - ODU', ...
                   'DomainName','Lower Chesapeake Bay','MinNumSites',MINSites,'MinNumRads',MINRadials,'graphics',1,'gridind',mind);
    end

    function createplotlistbutton_Callback(novar,novar1)
    ...
    function displaysettingsbutton_Callback(novar,novar1)
    ...
    function exportimagebutton_Callback(novar,novar1)
      myfilename = getfilename();
      if ~isempty(max(strfind(myfilename,'/')))
        try
          titlelab{pcount} = myfilename(max(strfind(myfilename,'/'))+1:end-4);
...
% SUPPORTING SUBFUNCTIONS
   ...
   function display_settings(novar,novar1)
...
function threddsbutton_Callback(novar,novar1)
   %Retrieve and plot THREDDS totals
   
   [LON,LAT,U,V,Uerr,Verr,NRAD]= get_thredds_data(dnum,gridval);
   notnan = find(~isnan(U+V));
   LON = LON(notnan);  LAT = LAT(notnan); U = U(notnan); V= V(notnan); Uerr = Uerr(notnan); Verr = Verr(notnan); NRAD = NRAD(notnan);
   if strcmp(get(ttool,'State'),'on'); 
     clear ZZ ZZZ
         if ~isempty(U) 
...
function cutoffbutton_Callback(novar,novar1) 
...
function gdopbutton_Callback(novar,novar1) 
    %Adjusts display settings.
...
function oibutton_Callback(novar,novar1) 
    %Adjusts display settings.
...
function rangeringbutton_Callback(novar,novar1) 
...
function savescalebutton_Callback(novar,novar1)
...
function loadnewsetup_Callback(novar,novar1)
...
function customgrid_Callback(novar,novar1)
...
function compareradbutton_Callback(novar,novar1)
